...

{
  ...
  method_list.forEach((item, ind) => {
    ...
    div.innerHTML = `<input type="radio" name="calculation-method__list--item" id="calculation-method__list--item--radio-${ind}" value="${
      item.value
    }" ${item.default ? "checked " : ""}/><label for="calculation-method__list--item--radio-${ind}">${item.name}</label>`
